<template>
    <div class="container mx-auto my-5">
        <UCard v-if="character">
            <template #header>
                <div class="flex justify-between items-center">
                    <h1 v-if="character" class="text-3xl font-bold">{{ character.name }}</h1>
                    <UButton
            ...
            <template #footer>
                <div v-if="character" class="mt-6 w-full">
                    ...
                    <div class="grid grid-cols-8 gap-2">
                        <div class="text-center rounded-md bg-indigo-600 px-3.5 py-2.5 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600" v-for="(episode, index) in character.episode" :key="index">Episode {{ episode.split('/').pop() }}</div>
                    </div>
                </div>
            </template>
        </UCard>
        ...
        <div v-if="error" class="text-center text-xl text-red-500">Error fetching character details.</div>
    </div>
</template>
